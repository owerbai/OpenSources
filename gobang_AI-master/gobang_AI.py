...
def ai():
    ...
    print(f"本次对方走{next_point[0],next_point[1]}")
    ...
    while g == 0:
        ...
        if change % 2 == 1:
            ...
            if game_win(list1):
                ...
                message.draw(win)
                with open('战绩.txt', 'w') as f:
                    f.write(f"{str(datetime.datetime.now())[:-5]}  Ower 战胜了 {name} ")
            ...
            if not ((round((p2.getX()) / GRID_WIDTH), round((p2.getY()) / GRID_WIDTH)) in list3):
                ...
                if game_win(list2):
                    ...
                    message.draw(win)
                    with open('战绩.txt','w+') as f:
                        f.write(f"{str(datetime.datetime.now())[:-6]}  Ower 输给了 {name} ")
